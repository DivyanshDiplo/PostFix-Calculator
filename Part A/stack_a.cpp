...
/*
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
----After this is optional i have added some tester code to test the calculator u can comment or delete this out according to your need--------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
int evaluatePostfix(const string& postfix) {
    ...
    while (ss >> token) {
        if (isdigit(token[0])) {
            ...
            if (stack.get_size() < 2) {
                throw runtime_error("Not enough operands for division");
            }
            int divisor = stack.get_element_from_top(0);
            if (divisor == 0) {
...
int main() {
    string postfix1 = "10 20 + 30 *";  // Expected output: 900
    string postfix2 = "50 10 5 * +";   // Expected output: 100
    string postfix3 = "100 0 / 5 +";  // Expected output: Not defined

    try {
        cout << "Evaluating postfix: " << postfix1 << endl;
        cout << "Result: " << evaluatePostfix(postfix1) << endl << endl;

        cout << "Evaluating postfix: " << postfix2 << endl;
        cout << "Result: " << evaluatePostfix(postfix2) << endl << endl;

        cout << "Evaluating postfix: " << postfix3 << endl;
        cout << "Result: " << evaluatePostfix(postfix3) << endl << endl;
